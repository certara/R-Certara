...
# Render merged capability workflows as markdown tables for the Cursor rule /
# WORKFLOW docs. Source of truth is certara_mcp_capabilities()$workflows
# (per-provider phases from each package's capabilities.json).
.mcp_render_workflow_phase_table <- function(workflows) {
  if (is.null(workflows) || !length(workflows)) {
    return(c(
      "| Phase | Tools |",
      "|-------|-------|",
      "| _(live map unavailable)_ | Call `certara_mcp_capabilities()$workflows` |"
    ))
  }
  .phase_row <- function(ph) {
    if (is.character(ph) && length(ph) == 1L) {
      return(sprintf("| %s | _(see provider tools)_ |", ph))
    }
    if (!is.list(ph)) {
      return(NULL)
    }
    id <- ph$id %||% ph$title %||% "phase"
    tools <- unlist(ph$tools, use.names = FALSE)
    tool_cell <- if (length(tools)) {
      paste(sprintf("`%s`", tools), collapse = ", ")
    } else {
      "_(none)_"
    }
    sprintf("| %s | %s |", id, tool_cell)
  }
  lines <- character(0)
  for (pkg in names(workflows)) {
    wf <- workflows[[pkg]]
    phases <- wf$phases %||% list()
    if (!length(phases)) next
    lines <- c(
      lines,
      sprintf("### %s", pkg),
      "",
    ...
    for (ph in phases) {
      row <- .phase_row(ph)
      if (!is.null(row)) lines <- c(lines, row)
    }
    cross <- wf$cross_cutting %||% list()
    if (length(cross)) {
      for (cc in cross) {
        if (is.character(cc) && length(cc) == 1L) {
          lines <- c(lines, sprintf("| %s (any phase) | _(see provider tools)_ |", cc))
          next
        }
        if (!is.list(cc)) next
        id <- paste0(cc$id %||% "cross", " (any phase)")
        tools <- unlist(cc$tools, use.names = FALSE)
        tool_cell <- if (length(tools)) {
          paste(sprintf("`%s`", tools), collapse = ", ")
        } else {
          "_(none)_"
        }
        lines <- c(lines, sprintf("| %s | %s |", id, tool_cell))
      }
    }
    lines <- c(lines, "")
  }
  if (!length(lines)) {
    return(c(
      "| Phase | Tools |",
      "|-------|-------|",
      "| _(no provider phases)_ | Call `certara_mcp_capabilities()$workflows` |"
    ))
  }
...
.write_cursor_mcp_rule <- function(path, server_name) {
  ...
  body <- gsub("__CALL_MCP_SERVER__", paste0("user-", server_name), body,
               fixed = TRUE)
  # Single-source the phase tables from merged capability fragments so the
  # Cursor rule cannot drift from certara_mcp_capabilities()$workflows.
  phase_table <- tryCatch({
    caps <- certara_mcp_capabilities()
    .mcp_render_workflow_phase_table(caps[["workflows"]])
  }, error = function(e) {
